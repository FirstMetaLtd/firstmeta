  ...
  $insertfile=$oldimage;
        ...
        $slug = preg_replace('/[^a-z0-9-]+/', '-', trim(strtolower($title)));
          ...
          $arr['id']=$_GET['id'];
            ...
            $addposts = "UPDATE `services` SET `title`=:title,`short_desc`=:short_desc,`description`=:description,`image`=:image,`slug`=:slug WHERE id=:id";
            $stm = $con->prepare($addposts);
            if($stm->execute($arr)){
                      echo "<script>swal('Success!', 'Service Updated successfully.', 'success').then(function() {
                                  window.history.go(-2);
                              })</script>";
            }
        }else{
